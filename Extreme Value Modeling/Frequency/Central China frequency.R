...
earthquake_data <- read_csv("/Users/WilsonWang/Desktop/Research/*Data earthquake/Central China.csv")
...
monthly_maxima <- earthquake_data %>%
  ...
  summarize(
    ...
    latitude = latitude[which.max(mag)],
    longitude = longitude[which.max(mag)]
  ) %>%
  ungroup()
