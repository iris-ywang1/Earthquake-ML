...
# Reading the data
eq.data <- read.csv(file="/Users/WilsonWang/Desktop/Research/*Data earthquake/Iran.csv", 
                    header=TRUE, sep=",")
...
# Add exponential distribution fit starting from the minimum value of int
curve(dexp(x - min(int), rate=1/mean(int - min(int))), col="blue", lwd=2, add=TRUE, from=min(int))
...
lambda <- 1 / mean(int - min(int))
exp_freq <- length(int) * diff(pexp(bin_edges - min(int), rate=lambda))
