...
# Reading the data
eq.data <- read.csv(file="/Users/WilsonWang/Desktop/Research/*Data earthquake/Philippines.csv", 
                    header=TRUE, sep=",")

# Filtering data for magnitude greater than 5.0
eq.data <- eq.data[eq.data$mag > 5.0, ]
...
# Add exponential distribution fit starting at the minimum value of interarrival times
min_int <- min(int)
curve(dexp(x - min_int, rate=1/mean(int - min_int)), col="blue", lwd=2, add=TRUE, from=min_int)
...
lambda <- 1 / mean(int - min_int)
exp_freq <- length(int) * diff(pexp(bin_edges - min_int, rate=lambda))
